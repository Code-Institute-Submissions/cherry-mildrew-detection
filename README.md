...
* As a data collector I must ensure that the data used is of sufficient quantity and quality. Garbage in results in garbage out. 

### Data visualisation, cleaning and preparation
* As a data user I must ensure images are resized to ensure a smoother push to GitHub
* As a data user I must create a plot to distinguish the contrast between powdery mildew leaves and normal leaves
...
* As a user I want a page listing all findings related to the study to visually differentiate a cherry leaf that is healthy and one that isn't
...
* As a user I want the Dashboard to be easy to use and clear to understand

## Hypothesis and how to validate?

### Hypothesis  

* We suspect that leaves that are infected by the powdery mildew have a lighter colour and patches across the leaf.
...
* Cherry leaves **without** Mildew - 0 healthy

* Cherry leaves **with** Mildew - 1 powdery mildew
...
* There will be an image montage showing all healthy leaves or a leaf containing powdery mildew 
...
* Build an ML model to predict future images with cherry leaves contained and classify them with a Neural Network that maps relationships. 


## ML Business Case
* From a given dataset of images, I must create an ML model to predict if a cherry leaf is healthy or has powdery mildew on it. It will be a fully supervised model.  
* The purpose of this is to speed up the time it takes to detect whether or not a leaf is healthy. Currently, it takes around 30 minutes on each tree to visually verify and then a further minute to apply a compound to kill the fungus.
...
* The client will benefit by not supplying the market with a product of poor quality through the creation of this model. 
...
1. Summary of the project, showing the project dataset summary and the client's requirements.  
...
4. My project hypothesis and how I validated the project  
...
* It will answer business requirement 1. 
   ...
   * Difference between an average healthy leaf and a leaf containing powdery mildew
   * Image Montage  


### Page 3: Mildew Detection 

* It will answer business requirements 2.
   * Link to download cherry leaves dataset
   * User interface to upload files/images. It will then display the image uploaded and include a prediction statement as to whether the image is a healthy leaf or contains cherry mildew.
   * Table with image name and prediction result
   Download button to download the table

### Page 4: Hypothesis and Validation

* The project hypothesis is shown in a success box, describing the conclusion and how it was validated.
...
## Unfixed Bugs
* All bugs were fixed as I went along. I did however create this repository after creating one earlier with the first and second notebooks already created. This was due to an issue when restarting my notebooks and the data being lost. I wasn’t able to resolve it on my old repository therefore I created this one and copied the work I had done previously onto this one, to ensure I met my project deadline. 
...
* NumPy  
     * Used to handle multi-dimensional arrays to operate on them including mathematical functions.
* pandas  
     * Used to structure the data in tables. Also used for data cleaning
* matplotlib 
     * Used to visualise the data (along with Seaborn)
* Seaborn 
     * Used to visualise the scatterplots/ bar plots and set themes and colour style
* Joblib
...
* Plotly
     * Used to create visualisations within Jupyter notebooks to present the data. Can be found on Mildew detector page on the dashboard

## Credits 

* A lot of this came from the First walkthrough project from the @codeinstitute program- Detecting Malaria.


### Content 

* Content on the summary page came from [Washington state Uni](http://treefruit.wsu.edu/crop-protection/disease-management/cherry-powdery-mildew/#:~:text=Powdery%20mildew%20of%20sweet%20and,1)
